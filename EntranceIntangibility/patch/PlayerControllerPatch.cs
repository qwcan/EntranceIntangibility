...
public class PlayerControllerPatch
{
    ...
    )
    {
        ...
        {
            //Plugin.Log.LogInfo( $"Player {__instance.playerClientId} is intangible, ignoring {damageNumber} damage from {causeOfDeath.ToString()}");
            return false;
        }
    ...
    private static void AllowPlayerDeathPatch(PlayerControllerB __instance, ref bool __result)
    {
        if( Plugin.Instance.IsIntangible( (int)__instance.playerClientId ) )
        {
            //Plugin.Log.LogInfo( $"Player {__instance.playerClientId} is intangible, ignoring death");
            __result = false;
        }
    }
